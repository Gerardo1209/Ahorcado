/*Equipo: Gerardo Femat Delgado y Emmanuel Muñoz Cerda*/
...
void participantes(char participante[]){
	...
	do{//Es ciclico ya que si el usuario ingresa mas caracteres de los que debe, lo devuelve a pedir su nombre.
...
void menu(){
	...
	do{ //Repite el menu hasta seleccionar la opcion de salir
		system("cls");
		printf("\n\t \t Menu de opciones\n\n");
...
void asignacionesPalabras(char palabra[],char tipo[]){ //Asigna las palabras a cada espacio del array de las distintas categorias.
	...
	switch((aleatorios(0)/10)){
		case 1: strcpy(palabra,colores[(aleatorios(dificultad)%10)]);
				strcpy(tipo,"Colores");
				break;
		case 2: strcpy(palabra,frutas[(aleatorios(dificultad)%10)]);
				strcpy(tipo,"Frutas");
				break;
		case 3: strcpy(palabra,series[(aleatorios(dificultad)%10)]);
				strcpy(tipo,"Series");
				break;
	}
}
...
int aleatorios(int dificultad){
	int num;
	srand(time(NULL));
	switch(dificultad){//Obtiene numeros aleatorios dependiendo de la dificultad que el usuario elige
		case 0: num=((rand()%30)+10); break;
		case 1: num=(rand()%3); break;
		case 2: num=((rand()%3)+4); break;
		case 3: num=((rand()%3)+7); break;
	}
	return num;
}
...
int juego(char participante[],char palabra[],char tipo[]){
	...
	do{
		...
		if(continuar==TAM){ //Si las palabras expuestas son iguales a los caracteres de la palabra quiere decir que el jugador ha ganado
			if(vida<6){ //Ocurre cuando el usuario comete algun error.
				imprAhorcado(vida=7,2,puntuaje,palabra);
			}
			else{ //Ocurre cuando el usuario no comete ning�n error!
				imprAhorcado(vida=7,3,puntuaje,palabra);
			}
			break;//Rompe el ciclo para que no se vuelva a mostrar que ingrese una palabra
		}
		do{ //Si el usuario introduce dos veces el mismo caracter el programa le dir� que introduzca uno nuevo
		...
		for(int i=0;i<TAM;i++){
			if(usadas[contUsadas]==palabra[i]){ //Compara la entrada con cada una de los caracteres de la palabra
...
void imprAhorcado(int vidas,int estado,int puntuacion,char palabra[]){
	system("cls");
	fflush(stdin);
	switch(vidas){//Imprime el sprite del ahorcado segun las vidas que le queden
	...
	switch(estado){//Envia mensajes al usuario, segun como este termine el juego
...
void imprUsadas(char usadas[]){
	...
	for (int i=0;i<strlen(usadas);i++){
		if(usadas[i]==' '||usadas[i]==13){//Cuando el ususario escribe un espacio o un enter(13) se imprime un guion(-).
			printf("- ");
		}
		else{
			printf("%c ",usadas[i]);
...
int imprPalabra(char palabra[], int comp[]){
	int continuar = 0;
	for (int i=0;i<strlen(palabra);i++){	//Imprime lo que ve el usuario que debe de adivinar
		if(i==0||i==(strlen(palabra)-1)||i==comp[i]){//Si la letra es la primera, ultima o la letra esta usada se exhibira la letra.
			printf(" %c ",palabra[i]);
			continuar++; //Suma continuar segun la cantidad de caracteres expuestos
		}
		else{//En caso contrario se imprimira un gui�n bajo denotando que faltan caracteres por descubrir
			printf(" _ ");
		}
	}
	return continuar;
...
bool comprobarUsadas(char usadas[],int nueva){
	...
	for(int i=0;i<nueva;i++){
		if(usadas[nueva]==usadas[i] || usadas[nueva]==13){//Si la letra que acaba de ingresar el usuario ya ha sido usada antes, el pograma le pide que escriba otra.
...
int controlVida(int incorrecto,int vida,int puntuacion,char palabra[]){
	if(incorrecto==0){
		--vida;
		if (vida==0){//Si el ususario tiene 0 vidas quiere decir que perdio y se debe de imprimir el ahorcado completo.
